...
### 第二步：创建自动化脚本文件
1. 在仓库中新建路径为 `.github/workflows/backup.yml` 的文件。
...
* **Error 404 (Not Found)**：
    1. 检查 KV 键名是否确实叫 `testUser`（注意大小写）。
    2. 检查 `WEBDAV_URL` 填写的文件夹是否已经在网盘中手动创建好。
* **Exit code 22**：代表网络请求失败。请通过 Action 的详细日志查看具体是下载阶段报错还是上传阶段报错。
