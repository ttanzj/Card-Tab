export async function onRequestPost(context) {
  ...
  if (url.pathname !== '/api/deleteLink') {
  ...
  try {
    ...
    parsedData.links = parsedData.links.filter(link => link.url !== linkUrl);
    
    for (const category in parsedData.categories) {
      parsedData.categories[category] = parsedData.categories[category].filter(link => link.url !== linkUrl);
    }
